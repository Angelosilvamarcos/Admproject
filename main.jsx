...
function Atendimentos(){const {rows,loading,error,updatedAt,reload}=useSolicitacoes();const [selected,setSelected]=useState(null),[saving,setSaving]=useState(false),[message,setMessage]=useState("");const update=async()=>{if(!selected||!supabase)return;setSaving(true);setMessage("");const {error}=await supabase.from("solicitacoes").update({status:selected.status,prioridade:selected.prioridade,categoria:selected.categoria||null,responsavel:selected.responsavel||null,resposta:selected.resposta||null,respondido_em:selected.resposta?.trim()?new Date().toISOString():null}).eq("id",selected.id);setSaving(false);if(error){setMessage(error.message);return}setSelected(null);setMessage("Atendimento atualizado com sucesso.");await reload()};return <section><div style={{display:"flex",justifyContent:"space-between",alignItems:"end",gap:16}}><div><h1>Atendimentos</h1><p className="muted">Gerencie solicitações, registre a solução e acompanhe o atendimento.</p></div><div style={{textAlign:"right"}}><button className="btn ghost" onClick={reload}>{loading?"Atualizando...":"Atualizar"}</button>{updatedAt&&<div className="muted" style={{fontSize:12,marginTop:6}}>Atualizado às {updatedAt.toLocaleTimeString("pt-BR")}</div>}</div></div>{(error||message)&&<div style={{marginTop:16,color:error?"#fca5a5":"#86efac"}}>{error||message}</div>}{loading?<div className="card" style={{padding:25,marginTop:25}}>Carregando solicitações...</div>:<div className="card" style={{padding:14,marginTop:25,overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr>{["Protocolo","Cliente","Canal","Assunto","Categoria","Status","Prioridade","Responsável","Ação"].map(h=><th key={h} style={{textAlign:"left",padding:12,borderBottom:"1px solid #29445a",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{rows.map(r=><tr key={r.id}>{[r.protocolo,r.nome_cliente,r.canal,r.assunto,r.categoria||"—",r.status,r.prioridade,r.responsavel||"—"].map((v,i)=><td key={i} style={{padding:12,borderBottom:"1px solid #1f3548",whiteSpace:"nowrap"}}>{v}</td>)}<td style={{padding:12,borderBottom:"1px solid #1f3548"}}><button className="btn primary" onClick={()=>setSelected({...r})}>Abrir</button></td></tr>)}</tbody></table>{rows.length===0&&<p className="muted" style={{padding:15}}>Nenhuma solicitação registrada.</p>}</div>}{selected&&<div className="card" style={{position:"fixed",inset:0,background:"#061522ee",display:"grid",placeItems:"center",padding:20,zIndex:20}}><div className="card" style={{padding:28,width:"min(700px,100%)",maxHeight:"90vh",overflowY:"auto"}}><div style={{display:"flex",justifyContent:"space-between"}}><div><h2 style={{marginTop:0}}>Atendimento {selected.protocolo}</h2><p className="muted">{selected.nome_cliente} · {selected.canal}</p></div><button className="btn ghost" onClick={()=>setSelected(null)}>Fechar</button></div><div className="card" style={{padding:16,margin:"18px 0",background:"#0b1e2d"}}><strong>{selected.assunto}</strong><p style={{whiteSpace:"pre-wrap",color:"#cbd5e1"}}>{selected.mensagem}</p></div><div><label className="label">Resposta / solução do atendimento</label><textarea className="input" rows="6" value={selected.resposta||""} onChange={e=>setSelected(s=>({...s,resposta:e.target.value}))} placeholder="Digite aqui a resposta ou solução que será registrada neste atendimento..."/></div><div className="grid grid2" style={{marginTop:18}}><div><label className="label">Status</label><select className="input" value={selected.status||""} onChange={e=>setSelected(s=>({...s,status:e.target.value}))}>{STATUS.map(x=><option key={x}>{x}</option>)}</select></div><div><label className="label">Prioridade</label><select className="input" value={selected.prioridade||""} onChange={e=>setSelected(s=>({...s,prioridade:e.target.value}))}>{PRIORIDADES.map(x=><option key={x}>{x}</option>)}</select></div><div><label className="label">Categoria</label><input className="input" value={selected.categoria||""} onChange={e=>setSelected(s=>({...s,categoria:e.target.value}))}/></div><div><label className="label">Responsável</label><input className="input" value={selected.responsavel||""} onChange={e=>setSelected(s=>({...s,responsavel:e.target.value}))}/></div></div>{selected.respondido_em&&<p className="muted" style={{marginTop:15}}>Resposta registrada em {new Date(selected.respondido_em).toLocaleString("pt-BR")}</p>}{message&&<div style={{marginTop:15,color:"#fca5a5"}}>{message}</div>}<button className="btn primary" style={{marginTop:20,width:"100%"}} disabled={saving} onClick={update}>{saving?"Salvando...":"Salvar atendimento e solução"}</button></div></div>}</section>}
